...
export class UserDataView {
    ...
    try {
      ...
      switch (currentModalMode) {
        ...
        case 'edit':
          ...
          break;
        case 'delete':
          if(this.currentFilter === false)  //user is being marked as Deleted
          {
            console.log('User is being marked as Deleted in category:', currentCategory, 'with data:', dataToSave);
            await this.dataService.updateData(currentCategory, dataToSave);
            break;
          }
          else{ //user is being hard Deleted
            console.log('User is being hard Deleted category:', currentCategory, 'with data:', dataToSave);
  ...
  filterItems()
  {
    ...
    this.filteredItems = this.loadedItems.filter( (item: any) => {
      return item.is_deleted === this.currentFilter;
    });
  }
